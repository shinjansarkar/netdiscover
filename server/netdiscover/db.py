...
class Database:
    ...
    def delete_scan(self, scan_id: str) -> bool:
        """
        Deletes a specific scan and all its related records (due to ON DELETE CASCADE).
        """
        with self.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute("DELETE FROM scans WHERE id = ?", (scan_id,))
            rows_deleted = cursor.rowcount
            cursor.execute("DELETE FROM hosts WHERE scan_id NOT IN (SELECT id FROM scans)")
            cursor.execute("DELETE FROM ports WHERE scan_id NOT IN (SELECT id FROM scans)")
            conn.commit()
            
        if rows_deleted > 0:
            self.insert_log("INFO", "Database", f"Scan {scan_id} deleted.")
            return True
        return False

    def delete_all_scans(self) -> int:
        """
        Deletes all scans and their related records.
        ...
        self.insert_log("WARNING", "Database", f"All scans deleted ({rows_deleted} total).")
        return rows_deleted
