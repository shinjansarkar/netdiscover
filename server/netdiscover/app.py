...
def api_health():
    """
    Public endpoint to verify the backend is online and accessible.
    """
    return jsonify({
        "status": "online",
        "message": "NetDiscover backend is up and running!",
...
def api_dashboard():
    ...
    return jsonify({
        "stats": stats,
        "scans": scans[:5], # Send only top 5 recent scans for dashboard
...
@app.route('/api/reports/<scan_id>/content')
def api_report_content(scan_id):
    json_path = os.path.join(REPORTS_DIR, f"report_{scan_id}.json")
    if os.path.exists(json_path):
        try:
            with open(json_path, 'r', encoding='utf-8') as f:
                return jsonify({"content": json.load(f)})
