...
class HostDiscovery:
    ...
    def run_arp_scan_on_subnet(self, subnet_cidr: str) -> List[Dict[str, Any]]:
        ...
        """
        discovered = []
        if not SCAPY_AVAILABLE or not hasattr(os, 'geteuid') or os.geteuid() != 0:
            return discovered
    ...
    def discover(self, subnet_cidr: str, method: str = "ICMP") -> List[Dict[str, Any]]:
        ...
        if method in ["ARP", "BOTH"] and SCAPY_AVAILABLE and hasattr(os, 'geteuid') and os.geteuid() == 0:
    ...
    def _get_arp_table_for_subnet(self, subnet_cidr: str) -> Dict[str, str]:
        ...
        try:
            ...
            # Fallback to `arp -a` on Windows, or `arp -n` on Unix
            cmd_args = ['-a'] if platform.system().lower() == 'windows' else ['-n']
            cmd = ['arp'] + cmd_args
            p = subprocess.run(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, timeout=2.0)
            out = p.stdout.strip().splitlines()
            for line in out:
    ...
    def get_mac_from_system(ip: str) -> Optional[str]:
        ...
        try:
            ...
            # Fallback to arp -a / arp -n
            cmd_args = ['-a', ip] if platform.system().lower() == 'windows' else ['-n', ip]
            cmd = ['arp'] + cmd_args
            p = subprocess.run(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, timeout=2.0)
            out = p.stdout.strip()
            # grep MAC like aa:bb:cc:dd:ee:ff or aa-bb-cc-dd-ee-ff
